...
Ilmo is a full-stack [GraphQL](https://graphql.org/learn/) and
...
For more details on what Ilmo is, see [the README](./README.md).
...
- **Modern JavaScript and GraphQL development:**
  ...
  - **GraphQL** — on the backend through PostGraphile, on the frontend via Urql
    client
  ...
  - **Realtime** — PostGraphile is configured with `@graphile/pg-pubsub` to
    enable Realtime events from the DB; and Urql is configured to consume them
